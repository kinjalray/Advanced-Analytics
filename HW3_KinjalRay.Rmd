...
#citibike$hourtime[is.na(citibike$hourtime)] <- 30
unique(citibike$hourtime)
...
citibike$timeOfDay <- ifelse(citibike$hourtime >= 04 & citibike$hourtime < 12, "Morning",ifelse(citibike$hourtime >= 12 & citibike$hourtime < 16, "Afternoon", ifelse(citibike$hourtime >= 16 & citibike$hourtime < 24, "Evening", "After Hours")))

library(ggplot2)

ggplot(citibike, aes(citibike$timeOfDay)) + geom_bar() + labs(x = "Time of Day")
